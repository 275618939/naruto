...
public class EmoticonsEditText extends EditText {
	...
	private Pattern buildPattern() {
		int size=NarutoApplication.getApp().mEmoticons.size();
		StringBuilder patternString = new StringBuilder(size * 3);
		patternString.append('(');
		for (int i = 0; i < size; i++) {
			String s = NarutoApplication.getApp().mEmoticons.get(i);
	...
	private CharSequence replace(CharSequence text) {
		try {
			...
			while (matcher.find()) {
				if (NarutoApplication.getApp().mEmoticonsId.containsKey(matcher.group())) {
					int id = NarutoApplication.getApp().mEmoticonsId.get(matcher.group());
