...
public class HttpUserUpdateService extends BaseService {

	public HttpUserUpdateService() {
		TAG="HttpUserUpdateService";
	}
	public HttpUserUpdateService(Context context) {
	...
	public void requestServer(CallBackService callbackService) {
		...
		try {
			...
			Object urlKey=params.get(URL_KEY);
			String url=Constant.User_Modify_API_URL;
			if(null!=urlKey){
				url=urlKey.toString();
			}
			
			String result = HttpUtils.requestPost(url, headers,params);
			if (result != null) {
				try {
