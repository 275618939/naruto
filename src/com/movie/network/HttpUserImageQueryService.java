...
public class HttpUserImageQueryService extends BaseService {

	public HttpUserImageQueryService() {
		TAG="HttpUserImageQueryService";
	}
	public HttpUserImageQueryService(Context context) {
	...
	public void requestServer(CallBackService callbackService) {
		...
		try {
			...
			StringBuilder path=new StringBuilder(Constant.User_Image_Query_API_URL);
			if(null!=memberId){
				path.append("/").append(memberId);
			}
			path.append("/").append(0).append("/").append(Page.MAX_SHOW_USER_PHOTO);
